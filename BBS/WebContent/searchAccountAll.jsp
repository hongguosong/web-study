...
<%
	...
	ResultSet sqlRst = sqlStmt.executeQuery("select count(*) from account_table where account_limit=0");
	if(sqlRst.next()){
			...
			<div id="mainSide" class="col-lg-8 col-sm-8">
				...
				<table class="table">
					...
					<tbody>
						...
						<%
							sqlRst=sqlStmt.executeQuery("select * from account_table where account_limit=0");
							while(sqlRst.next()){
						%>
								<tr>
									<td><%=sqlRst.getInt("account_account")%></td>
									<td><%=sqlRst.getString("account_name")%></td>
									<td><%=sqlRst.getString("account_pass")%></td>
									<td><%=sqlRst.getTimestamp("account_date").toLocaleString()%></td>
									<td>
										<a class="badge badge-pill badge-danger" href="account?reqType=deleteAccount&account_account=<%=sqlRst.getInt("account_account")%>">删除</a>
										<a class="badge badge-pill badge-info" href="account?reqType=updateAccount&account_account=<%=sqlRst.getInt("account_account")%>">修改</a>
										<a class="badge badge-pill badge-warning" href="account?reqType=setAccountAdmin&account_account=<%=sqlRst.getInt("account_account")%>">设为管理员</a>
									</td>								
								</tr>
						<%
							}
